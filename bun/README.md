...
|---|---|---|
| rustc, cargo | `src/ci/docker/host-*/dist-*-linux` + `src/tools/opt-dist` (how rustup's binaries are made: PGO'd LLVM, PGO'd rustc, BOLT on x86_64) | profiles gathered by compiling Bun instead of rustc-perf (`opt-dist --training-command`); BOLT on aarch64 too, not just x86_64 (needed a BOLT fix for ELF-compressed debug sections, in our llvm-project, and uncompressed debuginfo); rustc's LLVM has only the X86 and AArch64 backends; only the components Bun uses are dist'ed; docs, the cranelift backend and the post-dist test run are skipped; the host clang/lld/llvm-bolt are built from the same llvm-project commit as the one shipped (upstream: a pinned older clang) against a GCC 13 libstdc++ (upstream: GCC 9) |
...
`https://github.com/oven-sh/rust/compare/574ff7d98bd6d037e5236a8453029173b32631fd...nightly-2026-09-15-bun`
(the nightly's rust-lang/rust commit, then this branch).
...
bun-toolchain-<host>-<variant>-rust.tar.zst
  bin/   rustc cargo rustfmt cargo-clippy (trained variants also: rustdoc cargo-miri)
...
or newer). libstdc++ (GCC 13's), zstd and libxml2 are linked statically into rustc's libLLVM.so
and into clang and lld; zlib dynamically, as the upstream release binaries do. The macOS toolchain targets macOS ≥ 13 and depends only on system
...
```sh
git clone --branch nightly-2026-09-15-bun https://github.com/oven-sh/rust && cd rust
...
node bun/toolchain.ts all --variant=dev --host-llvm=/usr/lib/llvm-23   # any LLVM ≥ the one being built
...
When Bun moves to a new nightly: branch `nightly-<date>-bun` from that nightly's
rust-lang/rust commit (`rustc +nightly-<date> -vV` prints it), cherry-pick this
branch's commits, update the compare link above, and point `src/llvm-project` at the
