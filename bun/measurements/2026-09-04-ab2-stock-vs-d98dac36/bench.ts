...
// save their [timing] lines, next.
import { execFileSync } from "node:child_process";
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
import { join } from "node:path";

const REPO = `${process.env.HOME}/code/bun-toolchain-trial`;
const OUT = `${process.env.HOME}/.cache/bun-toolchain-test/ab2`;
...
for (let i = done().length; i < RUNS; i++) {
  ...
  git("commit", "-q", "--allow-empty", "-m", `ci (trial): toolchain A/B #2 run ${i + 1}/${RUNS} — ${arm}`);
