...
/** x86_64's RUST_CONFIGURE_ARGS name the host clang's CMake package, for the offload component. */
const OFFLOAD_CLANG_DIR = (o: Options) => `--set llvm.offload-clang-dir=${o.hostLlvm}/lib/cmake/clang`;
...
function dockerfileConfigureArgs(o: Options): string[] {
  ...
  const common = [
    ...
    ...(o.host === "linux-x64" ? [OFFLOAD_CLANG_DIR(o)] : []),
 ...
 * and the host compilers in bun/Dockerfile (upstream: self-built clang 23.1.0-rc1, GCC 9's
 * libstdc++.a; here: clang built from src/llvm-project's commit, GCC 13's libstdc++.a).
 */
export function bunDeltas(o: Options): { drop: string[]; add: string[]; env: Record<string, string> } {
  ...
  return {
    drop: [
      ...
      OFFLOAD_CLANG_DIR(o), // build only: read when llvm.offload is on; offload is a separate component we do not ship
...
export function plainConfigureArgs(o: Options): string[] {
  ...
  return [
    ...
    ...(isWindows(o) ? [] : ["--set rust.lto=thin", "--set build.allocator=jemalloc"]),
    "--set rust.codegen-units=1",
    "--set rust.codegen-backends=llvm",
    "--set build.extended=true",
    // The extended tools in PLAIN_COMPONENTS, which leaves rustdoc out of the rustc component
    // (unset, the list means "all"). Nothing in Bun's build runs rustdoc, and it is the one thing
    // here that depends on psm, whose GAS fallback clang-cl cannot assemble for aarch64-pc-windows-msvc.
    "--set build.tools=cargo,clippy,rustfmt",
...
export function installDist(o: Options, which: string[] = SHIPPED_COMPONENTS): void {
  const p = paths(o);
