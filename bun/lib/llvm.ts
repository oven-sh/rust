...
//   clang/cmake/caches/Release.cmake          (the cache file)
...
export const DISTRIBUTION_COMPONENTS = [
  ...
  // every binutils-style tool a Bun build (any target, incl. the Mach-O and PE/COFF steps) or a
  // developer inspecting its output reaches for, so nothing falls back to a system LLVM
  "llvm-ar", "llvm-ranlib", "llvm-lib", "llvm-dlltool", "llvm-nm", "llvm-strings", "llvm-size",
...
function everyStageCache(o: Options): Record<string, string> {
  return {
    // Upstream builds every backend (Release.cmake makes stage 1 Native-only); Bun targets
    // x86_64 and aarch64.
    ...
    // on a host libxml2.so — its soname differs between distros. (Release.cmake asks for a static
    // libxml2 too, since LLVM 23, with LLVM_USE_STATIC_LIBXML2; here the archive is named outright.)
...
function instrumentedCache(o: Options): Record<string, string> {
  const cache: Record<string, string> = {
    // Release.cmake inputs; they size stage 1 and the instrumented stage. Upstream:
...
function finalStageCache(o: Options): Record<string, string> {
  ...
  // Release.cmake's RELEASE_LINKER_FLAGS on Linux (what BOLT needs from the link).
  ...
  const cache: Record<string, string> = {
    ...
    // Release.cmake, passed through every stage.
    CMAKE_BUILD_TYPE: "Release",
    // set_instrument_and_final_stage_var
    CMAKE_POSITION_INDEPENDENT_CODE: "ON",
    LLVM_ENABLE_LTO: "Thin",
    LLVM_ENABLE_LLD: "ON",
    // RELEASE_LINKER_FLAGS (the executables' are below, with our additions)
    ...
    // CLANG_BOLT=INSTRUMENT upstream; ours is boltWithBun() (deviation: workload, and
    // lld too).
    CLANG_BOLT: "OFF",
    // set_final_stage_var
    ...
    // runs `package` over everything (LLVM_RELEASE_FINAL_STAGE_TARGETS).
