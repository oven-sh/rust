...
//                        OPT_DIST_PROFILES (which rustc-perf profile kinds the step wants: the
//                        compile ones, Doc, or Clippy) and OPT_DIST_PHASE (pgo | bolt)
...
import { type BunBuild, bunBuild, checkoutBun, ninja, type Toolchain } from "./lib/bun-build.ts";
...
function trainRust(): void {
  ...
  if (profiles.includes("Clippy")) clippy(rustc, cargo, toolchain);
}

// Bun's `rust:clippy` (package.json), set up as its rust-lints workflow does: the generated
// sources and the two vendored Rust crates, then clippy over the workspace. cargo-clippy and
// clippy-driver are the instrumented ones next to rustc.
function clippy(rustc: string, cargo: string, toolchain: Toolchain): void {
  const dir = bunBuild(b, `clippy-${v.name}`, t.target, t.args, toolchain, null);
  for (const target of ["codegen", "clone-lolhtml", "clone-rust-argon2"]) ninja(b, dir, target);
  run([cargo, "clippy", "--workspace", "--no-deps", "--keep-going"], {
    cwd: config.bunDir,
    env: {
      RUSTC: rustc,
      PATH: `${dirname(rustc)}:${process.env.PATH}`,
      BUN_CODEGEN_DIR: join(dir, "codegen"),
      CARGO_TARGET_DIR: join(dir, "clippy-target"),
      RUSTFLAGS: undefined,
