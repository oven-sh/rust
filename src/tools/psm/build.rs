...
fn main() {
    ...
    // The GAS-syntax files go through the C preprocessor (psm.h and the CFG_TARGET_* defines
    // pick the directives for the target). A cl-like compiler (clang-cl, when cross-compiling to
    // *-windows-msvc from elsewhere, where MASM is not an option) has no -x flag; it preprocesses
    // assembly by extension instead, so hand it the file as .S.
    let gas = asm.ends_with(".s");
    if gas {
        if !msvc {
    ...
    if asm.ends_with(".o") {
        ...
        let out = std::path::PathBuf::from(var("OUT_DIR").unwrap());
        let src = std::path::Path::new(asm);
        let copy = out.join(src.file_name().unwrap()).with_extension("S");
        std::fs::copy(src, &copy).expect("copy assembly to OUT_DIR");
        cfg.include(src.parent().unwrap());
        cfg.file(copy);
    } else {
        cfg.file(asm);
    }
